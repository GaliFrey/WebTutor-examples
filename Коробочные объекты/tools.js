 ...
 * @param event_id - Тип: Целое число. ID мероприятия, для участников которого назначается курс.
 * @param course_id - Тип: Целое число. ID курса, который необходимо назначить.
 * @param [doc_event] - Тип: Объект XmlDoc. Документ карточки мероприятия.
 * @param [duration] - Тип: Целое число. Длительность курса в днях. Определяет дату планируемого завершения.
 * @param [start_learning_date] - Тип: Дата. Дата начала обучения. Если данный аргумент задан, то обучение невозможно будет начать раньше указанной даты.
 * @param [last_learning_date] - Тип: Дата. Контрольная дата завершения предыдущего обучения. Если параметр задан, то при назначении курса, проверяется, существует ли в каталоге learnings курс, завершенный после указанной даты. Если такой курс существует, то соответствующая запись из каталога learnings возвращается как результат работы функции.
 * @return Тип: Целое число. Количество назначенных курсов.
 */
tools.activate_course_to_event = function(event_id, course_id, doc_event, duration, start_learning_date, last_learning_date) {}

/**
 * Назначение курса сотруднику.
 * @param personID - Тип: Целое число. ID сотрудника, которому назначается курс.
 * @param course_id - Тип: Целое число. ID курса, который необходимо назначить.
 * @param [event_id] - Тип: Целое число. ID мероприятия, участникам которого назначается курс.
 * @param [person_doc] - Тип: TopElem объекта XmlDoc. TopElem карточки сотрудника, которому назначается курс.
 * @param [education_plan_id] - Тип: Целое число. ID плана обучения, в рамках которого назначен курс.
 * @param [duration] - Тип: Целое число. Длительность курса в днях. Определяет дату планируемого завершения.
 * @param [start_learning_date] - Тип: Дата. Дата начала обучения. Если данный аргумент задан, то обучение невозможно будет начать раньше указанной даты.
 * @param [dtLastLearningDateParam] - Тип: Дата. Контрольная дата завершения предыдущего обучения. Если параметр задан, то при назначении курса, проверяется, существует ли в каталоге learnings курс, завершенный после указанной даты. Если такой курс существует, то ID соответствующей записи из каталога learnings возвращается как результат работы функции.
 * @param [group_id] - Тип: Целое число. ID группы, которой назначен курс.
 * @param [sEIDParam] - Тип: Строка. Код записи в каталоге active_learnings. Если он указан, то при назначении курса, когда производится проверка на уже существующий активный курс данного сотрудника в каталоге active_learnings, проверяется также, что у данной записи должен быть указанный в параметре код.
 * @param [bSkipDismissedParam] - Тип: Булево. Аргумент, указывающий на необходимость пропускать уволенных сотрудников (true – пропускать, false –не пропускать). По умолчанию true.
 * @return Тип: Объект XMLDoc или Целое число. Если курс назначен при выполнении функции, то возвращается ссылка на вновь созданный документ обучения. Если курс был назначен ранее, но не завершен, или завершен, но не прошло еще время, указанное в атрибуте dtLastLearningDateParam, то возвращается ID карточки ранее назначенного курса (из каталога active_learning).
 */
tools.activate_course_to_person = function(personID, course_id, event_id, person_doc, education_plan_id, duration, start_learning_date, dtLastLearningDateParam, group_id, sEIDParam, bSkipDismissedParam) {}
